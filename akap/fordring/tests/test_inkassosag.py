...
class BasicTestCase(TestMixin, TestCase):
    def setUp(self):
        ...
        self.prisme_return = {
            "PrismeClaimRequest": PrismeClaimResponse(
                None,
    ...
    def test_create_claim_request_parse(self):
        attachment = File(open("fordring/tests/resources/testfile.pdf"))
        attachment.close()
        ...
        self.compare(
            xml_to_dict(
                self.get_file_contents("fordring/tests/resources/claim_request.xml")
            ),
    ...
    def test_create_claim_response_parse(self):
        response = PrismeClaimResponse(
            None, self.get_file_contents("fordring/tests/resources/claim_response.xml")
        )
    ...
    def test_claim_upload_success(self):
        # Contains all required fields, and some more
        fp = open("fordring/tests/resources/inkasso.csv", "rb")
        uploadfile = SimpleUploadedFile("test.csv", fp.read(), content_type="text/csv")
        fp.close()
