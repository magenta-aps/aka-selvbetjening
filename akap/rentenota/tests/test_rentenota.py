...
class BasicTestCase(TestMixin, TestCase):
    def setUp(self):
        ...
        session.save()
        self.prisme_return = {
            "PrismeInterestNoteRequest": PrismeInterestNoteResponse(
                None,
                self.get_file_contents(
                    "rentenota/tests/resources/interestnote_response.xml"
                ),
            )
        }
    ...
    def test_interest_note_request_parse(self):
        ...
        self.compare(
            xml_to_dict(
                self.get_file_contents(
                    "rentenota/tests/resources/interestnote_request.xml"
                )
            ),
    ...
    def test_interestnote_success(self):
        response = self.client.get(self.url, {"year": 2019, "month": 10})
        self.assertEqual(response.status_code, 200)
        root = etree.fromstring(response.content, etree.HTMLParser())
        header_row = root.xpath("//table[@class='output-table']/thead/tr")[0]
        headers = [
            cell.text.strip(" \n").replace("\xad", "&shy;")
            for cell in header_row.iterchildren()
        ]
        rows = root.xpath(
            "//table[@class='output-table']/tbody/tr[@class='rentenota-post-table-datarow']"
        )
        data = [
            {
                headers[i]: cell.text.strip(" \n")
                for i, cell in enumerate(row.iterchildren())
            }
            for row in rows
        ]
        ...
        rows = root.xpath(
            "//table[@class='output-table']/tbody/tr[@class='rentenota-post-table-sumrow']"
        )
    ...
    def test_invalid_cvr(self):
        ...
        for y in range(2000, 2019):
            for m in range(1, 13):
                response = self.client.get(self.url, {"year": y, "month": m})
