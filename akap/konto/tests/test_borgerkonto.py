...
class BasicTestCase(TestMixin, TestCase):
        ...
        self.prisme_return = {
            "PrismeAccountRequest": PrismeAccountResponse(
                ...
                self.get_file_contents(
                    "konto/tests/resources/citizenaccount_response.xml"
                ),
            )
        }
    ...
    def test_account_request_parse(self):
        ...
        self.compare(
            xml_to_dict(
                self.get_file_contents(
                    "konto/tests/resources/citizenaccount_request.xml"
                )
            ),
    ...
    def test_account_response_parse(self):
        response = PrismeAKIAccountResponse(
            None,
            self.get_file_contents("konto/tests/resources/citizenaccount_response.xml"),
        )
