...
class LocalTestCase(BasicTestCase):
    ...
    def test_impairment_request_parse(self):
        ...
        self.compare(
            xml_to_dict(
                self.get_file_contents("aka/tests/resources/impairment_request.xml")
            ),
    ...
    def test_impairment_response_parse(self):
        response = PrismeImpairmentResponse(
            None, self.get_file_contents("aka/tests/resources/impairment_response.xml")
        )
    ...
    def test_impairment_invalid_amount(self):
        ...
        erroritems = root.xpath(
            "//div[@data-field='id_beloeb']//ul[@class='errorlist']/li"
        )
    ...
    def test_impairment_missing_debitor(self):
        # Contains just the required fields
        formData = {"beloeb": 123, "ekstern_sagsnummer": "1234", "sekvensnummer": "1"}
        response = self.client.post(self.url, formData)
        self.assertEqual(response.status_code, 200)
        root = etree.fromstring(response.content, etree.HTMLParser())
        erroritems = root.xpath(
            "//div[@data-field='id_debitor']//ul[@class='errorlist']/li"
        )
        self.assertEqual(1, len(erroritems))
        self.assertEqual("error.required", erroritems[0].attrib.get("data-trans"))
    ...
    def test_impairment_missing_casenumber(self):
        # Contains just the required fields
        formData = {"debitor": "test-debitor", "beloeb": 123, "sekvensnummer": "1"}
        response = self.client.post(self.url, formData)
        self.assertEqual(response.status_code, 200)
        root = etree.fromstring(response.content, etree.HTMLParser())
        erroritems = root.xpath(
            "//div[@data-field='id_ekstern_sagsnummer']//ul[@class='errorlist']/li"
        )
        self.assertEqual(1, len(erroritems))
        self.assertEqual("error.required", erroritems[0].attrib.get("data-trans"))
    ...
    def test_impairment_missing_sequencenumber(self):
        ...
        root = etree.fromstring(response.content, etree.HTMLParser())
        erroritems = root.xpath(
            "//div[@data-field='id_sekvensnummer']//ul[@class='errorlist']/li"
        )
        self.assertEqual(1, len(erroritems))
        self.assertEqual("error.required", erroritems[0].attrib.get("data-trans"))
