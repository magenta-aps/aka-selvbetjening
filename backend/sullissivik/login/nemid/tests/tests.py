...
test_settings = {
    ...
    'DEFAULT_CPR': None,
    'NEMID_CONNECT': {
        'federation_service': "https://qaip.sullissivik.gl/services/IdentityServiceV2.svc?wsdl",
        'cookie_name': "QA.Sullissivik.Federation.Cookie",
        'cookie_path': "/",
        'cookie_domain': ".sullissivik.gl",
        'login_url': "https://qaip.sullissivik.gl/login.aspx",
        'redirect_field': "returnurl"
    },
    'LOGIN_PROVIDER_CLASS': 'sullissivik.login.nemid.nemid.NemId',
}


@override_settings(**test_settings)
class LoginTestCase(TestCase):
    config = test_settings['NEMID_CONNECT']
    sfc = "QA.Sullissivik.Federation.Cookie"
    ...
    def testValidCookieDoesRedirect2(self):
        ...
        self.assertIn("%s?sc_lang=%s&%s=%s" % (
            self.config['login_url'],
            "da-DK",
            self.config['redirect_field'],
            urlquote(returnurl)
        ), location)
